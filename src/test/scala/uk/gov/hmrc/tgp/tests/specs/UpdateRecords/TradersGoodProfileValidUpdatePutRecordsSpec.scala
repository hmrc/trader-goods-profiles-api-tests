...
class TradersGoodProfileValidUpdatePutRecordsSpec extends BaseSpec with CommonSpec with HttpClient {

  object UpdatePutApiRecord extends Tag("uk.gov.hmrc.tgp.tests.specs.TradersGoodProfileValidUpdatePutRecordsSpec")

  Feature("Traders Good Profile API functionality for Valid Update PUT Record API call") {
    ...
    val payloads = Map(
      ...
      "MandatoryPayload"          -> getRequestJsonFileAsString(FolderName, "Scenario_UpdatePut_200_OnlyMandatory"),
    ...
    runScenario(
      ValidEori,
      "Scenario_UpdatePut_200_OnlyMandatory",
      200,
      payloads("MandatoryPayload"),
      "Validate success 200 for Update PUT TGP record API with only Mandatory values"
    )
    runScenario(
      ValidEori,
